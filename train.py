...
def train():
    ...
    env_name = "InvertedPendulum-v4"  # For quick runs and many visuals

    has_continuous_action_space = True

    max_ep_len = 1000
    max_training_timesteps = int(2.5e5)  # <2 hours

    print_freq = 5000  # Frequent prints
    log_freq = 500     # Denser logs
    save_model_freq = int(1e4)  # More checkpoints for GIFs/visuals
    ...
    hidden_size = 256  # For InvertedPendulum
    replay_size = int(1e6)
    batch_size = 256
    updates_per_step = 1
    start_steps = 5000
    ...
    while time_step <= max_training_timesteps:
        ...
        while True:
            ...
            if time_step % print_freq == 0:
                ...
                elapsed = (datetime.now().replace(microsecond=0) - start_time).total_seconds()
                if time_step > 0:
                    steps_per_sec = time_step / elapsed
                    remaining_steps = max_training_timesteps - time_step
            ...
            if time_step % save_model_freq == 0:
                ...
                # Update for next estimation
                if last_checkpoint_timestep > 0:
                    time_diff = (current_time - last_checkpoint_time).total_seconds()
                    step_diff = time_step - last_checkpoint_timestep
                    if time_diff > 0:
                        ...
                        hours = int(remaining_time // 3600)
                        minutes = int((remaining_time % 3600) // 60)
                        print(f"Estimated time remaining (based on last interval): {hours}h {minutes}m")

                last_checkpoint_time = current_time
                last_checkpoint_timestep = time_step
